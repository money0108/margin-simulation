...
| 標的類型 | 槓桿倍數 | 判定依據 |
|---------|---------|---------|
| ETF（月均量 ≥ 10,000 張） | 7x | 依 ETF 月均成交量判定 |
| ETF（月均量 < 10,000 張） | 5x | 依 ETF 月均成交量判定 |
| 股期標的（股期槓桿 > 7） | 5x | 依期貨槓桿倍數動態判定 |
| 股期標的（股期槓桿 > 6 且 ≤ 7） | 4x | 依期貨槓桿倍數動態判定 |
| 股期標的（股期槓桿 ≤ 6） | 3x | 依期貨槓桿倍數動態判定 |
| 其他股票 | 3x | 非股期標的 |
